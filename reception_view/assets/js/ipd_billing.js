...
const billing = (function () {
    ...
    function toggleGroup(groupId) {
        ...
        // Check if currently hidden based on class or computed display
        const firstRow = rows[0];
        const isHidden = firstRow.classList.contains('is-hidden') || 
                         firstRow.style.display === 'none' || 
                         (window.getComputedStyle && window.getComputedStyle(firstRow).display === 'none');
    ...
    function renderItemsTable(items) {
        ...
        Object.values(grouped).forEach(group => {
            ...
            group.items.forEach((item, index) => {
                ...
                html += `
                    <tr class="child-row is-hidden group-${safeId} ${isCancelled ? 'cancelled-row' : ''}" data-group-id="${safeId}" style="display: none !important; background: #faf8f5; border-bottom: 1px solid rgba(31, 107, 74, 0.15); color: #1f6b4a;">
