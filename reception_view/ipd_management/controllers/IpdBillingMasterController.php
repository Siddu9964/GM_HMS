...
class IpdBillingMasterController extends BaseController {
    protected $model;
    ...
    protected function handlePost(): void {
        ...
        switch ($action) {
            ...
            case 'add_payment':
                ...
                if ($amount <= 0) { $this->error('Amount must be positive', 400); return; }
                $mode = $data['payment_mode'] ?? 'CASH';
                $type = $data['payment_type'] ?? 'PARTIAL';
                $ref  = $data['reference'] ?? '';
                $result = $this->model->addPayment($data['bill_id'], $amount, $user, $mode, $type, $ref);
                $this->success($result, 'Payment recorded successfully');
                break;
