...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'add_bed_to_room') {
            ...
            $dup = $db->fetchOne("SELECT sl_no FROM hospital_beds WHERE room_number = ? AND bed_number = ?", [$room_number, $newBedNumber]);
            if ($dup) {
                $newBedNumber = "{$room_number}-" . ($count + 1);
            }
        ...
        if ($action === 'update' || $action === 'edit') {
            ...
            $targetBedNumber = !empty($bed_number) ? $bed_number : $existing['bed_number'];
            $targetRoomNumber = !empty($room_number) ? $room_number : $existing['room_number'];

            // Check if room_number + bed_number exists on a different bed
            $dup = $db->fetchOne(
                "SELECT sl_no FROM hospital_beds WHERE room_number = ? AND bed_number = ? AND sl_no != ?",
                [$targetRoomNumber, $targetBedNumber, $sl_no]
            );
            if ($dup) {
                echo json_encode([
                    'status' => 'error',
                    'message' => "Bed '{$targetBedNumber}' already exists in Room {$targetRoomNumber}."
                ]);
                exit;
            }
            ...
            $updatePayload = [
                ...
                'room_name'        => $room_name ?: $targetRoomNumber,
            ...
            echo json_encode(['status' => 'success', 'message' => "Bed {$targetBedNumber} updated successfully."]);
            exit;
        }
        ...
        if ($batchCount > 1) {
            ...
            for ($i = 0; $i < $batchCount; $i++) {
                ...
                $dup = $db->fetchOne("SELECT sl_no FROM hospital_beds WHERE room_number = ? AND bed_number = ?", [$room_number, $currentBedNum]);
                if ($dup) {
                    continue;
                }
            ...
            $dup = $db->fetchOne("SELECT sl_no FROM hospital_beds WHERE room_number = ? AND bed_number = ?", [$room_number, $bed_number]);
            if ($dup) {
                echo json_encode(['status' => 'error', 'message' => "A bed with number '{$bed_number}' already exists in Room {$room_number}."]);
                exit;
            }
        ...
        if (strpos($msg, 'Duplicate entry') !== false) {
            $msg = 'A bed with this number already exists in this room.';
        }
