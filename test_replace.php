<?php
$filePath = 'd:/xampp/htdocs/GM_HMS/reception_view/ipd_management/views/beds/index.php';
...
$newScript = <<<'EOD'
        ...
        // Navigation State
        let currentView = {
            level: 'hospital', // 'hospital', 'floor', 'ward', 'roomType', 'room'
            floor: null,
            ward: null,
            roomType: null,
            room: null
        ...
        function buildHierarchy(beds) {
            ...
            beds.forEach(bed => {
                ...
                if (!hospitalData.floors[fName].wards[wName].roomTypes[rType]) {
                    hospitalData.floors[fName].wards[wName].roomTypes[rType] = {
                        name: rType,
                        rooms: {},
                        stats: { total:0, occ:0, avail:0 }
                    };
                    uniqueRoomTypes.add(fName + '_' + wName + '_' + rType);
                }

                // Ensure Room exists
                if (!hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum]) {
                    hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum] = {
                        number: rNum,
                        name: bed.room_name,
                        type: rType,
                        beds: [],
                    ...
                    uniqueRooms.add(fName + '_' + wName + '_' + rType + '_' + rNum);
                }

                // Append Bed
                const bedObj = { ...bed, normalized_status: normStatus };
                hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum].beds.push(bedObj);

                // Aggregate Stats
                hospitalData.stats.totalBeds++;
                hospitalData.floors[fName].stats.total++;
                hospitalData.floors[fName].wards[wName].stats.total++;
                hospitalData.floors[fName].wards[wName].roomTypes[rType].stats.total++;
                hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum].stats.total++;
                ...
                if (normStatus === 'Occupied') {
                    ...
                    hospitalData.floors[fName].wards[wName].roomTypes[rType].stats.occ++;
                    hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum].stats.occ++;
                    ...
                    hospitalData.floors[fName].wards[wName].roomTypes[rType].stats.avail++;
                    hospitalData.floors[fName].wards[wName].roomTypes[rType].rooms[rNum].stats.avail++;
            ...
            hospitalData.stats.totalRoomTypes = uniqueRoomTypes.size;
        ...
        function navigateTo(level, fName, wName = null, rType = null, rNum = null) {
            ...
            if (level === 'floor') {
                ...
                renderRoomTypes(fName, wName);
            } else if (level === 'roomType') {
                renderRooms(fName, wName, rType);
            } else if (level === 'room') {
                renderBeds(fName, wName, rType, rNum);
            }
        }
        ...
        function renderBreadcrumbs() {
            ...
            if (currentView.roomType) {
                html += `<i class="fas fa-chevron-right breadcrumb-separator"></i>`;
                const act = currentView.level === 'roomType' ? 'active' : '';
                html += `<div class="breadcrumb-item ${act}" onclick="navigateTo('roomType', '${currentView.floor}', '${currentView.ward}', '${currentView.roomType}')">${currentView.roomType}</div>`;
            }
            if (currentView.room) {
                html += `<i class="fas fa-chevron-right breadcrumb-separator"></i>`;
                ...
                html += `
                    <div class="premium-card searchable-card" data-search="${ward.name.toLowerCase()}" onclick="navigateTo('ward', '${floorName}', '${ward.name}')">
                        <div class="card-header">
                            ...
                            <span class="bed-badge" style="background:var(--bed-bg); color:var(--bed-primary)">${totalRooms} Rooms</span>
                        </div>
        ...
        function renderRoomTypes(floorName, wardName) {
            ...
            Object.values(ward.roomTypes).forEach(rType => {
                const occPct = rType.stats.total > 0 ? Math.round((rType.stats.occ / rType.stats.total) * 100) : 0;
                const roomsCount = Object.keys(rType.rooms).length;
                
                html += `
                    <div class="premium-card searchable-card" data-search="${rType.name.toLowerCase()}" onclick="navigateTo('roomType', '${floorName}', '${wardName}', '${rType.name}')">
                        <div class="card-header">
                            <div class="card-title"><i class="fas fa-bed"></i> ${rType.name}</div>
                            <span class="bed-badge" style="background:var(--bed-bg); color:var(--bed-primary)">${roomsCount} Rooms</span>
                        </div>
                        <div class="card-stats">
                            <div class="card-stat-item">
                                <div class="card-stat-val occ">${rType.stats.occ}</div>
                                <div class="card-stat-label">Occupied</div>
                            </div>
                            <div class="card-stat-item">
                                <div class="card-stat-val ava">${rType.stats.avail}</div>
                                <div class="card-stat-label">Available</div>
                            </div>
                        </div>
                        <div style="font-size:0.75rem; color:var(--bed-text-muted); display:flex; justify-content:space-between; margin-top:1rem;">
                            <span>Total Beds: ${rType.stats.total}</span>
                            <span>${occPct}% Full</span>
                        </div>
                        <div class="progress-bar-container">
                            <div class="progress-bar-fill" style="width: ${occPct}%; ${occPct > 80 ? 'background:var(--status-occ)' : ''}"></div>
        ...
        function renderRooms(floorName, wardName, roomTypeName) {
            const rType = hospitalData.floors[floorName].wards[wardName].roomTypes[roomTypeName];
            if (!rType) return;

            let html = `<div class="grid-layout">`;
            
            Object.values(rType.rooms).forEach(room => {
                const occPct = room.stats.total > 0 ? Math.round((room.stats.occ / room.stats.total) * 100) : 0;
                
                html += `
                    <div class="premium-card searchable-card" data-search="room ${room.number.toLowerCase()} ${room.name ? room.name.toLowerCase() : ''}" onclick="navigateTo('room', '${floorName}', '${wardName}', '${roomTypeName}', '${room.number}')">
                        <div class="card-header">
...
if ($startPos !== false && $endPos !== false) {
    $newContent = substr($content, 0, $startPos) . $newScript . "\n    " . substr($content, $endPos);
    file_put_contents($filePath, $newContent);
    echo "Replaced script in index.php successfully!\n";
} else {
