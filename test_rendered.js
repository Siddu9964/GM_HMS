            ...
            IPD.ajax('beds', 'GET')
                .then(response => {
                    try {
                            ...
                            if(floorKeys.length > 0) {
                                ...
                                $('#appDynamicView').html(`
                                    <div class="empty-state">
                                        ...
                                        <h3>No Beds Found</h3>
                                        <p>There are no beds available in the system.</p>
                                    </div>
                                `);
                                $('#floorList').html('<div style="padding:1rem;text-align:center;">No floors found</div>');
                            }
                        } else {
                        ...
                        console.error(e);
                        alert("JS Render Error: " + e.message);
                        $('#floorList').html(`<div style="color:red; padding:1rem;">Error rendering: ${e.message}</div>`);
                    }
                })
                .catch(error => {
                    console.error("API Error: ", error);
                    let msg = error.message || 'Failed to load bed data';
                    IPD.toast(msg, 'error');
                    $('#floorList').html(`<div style="color:red; padding:1rem; text-align:center;"><i class="fas fa-exclamation-triangle"></i><br>API Error:<br>${msg}</div>`);
                });
        }
